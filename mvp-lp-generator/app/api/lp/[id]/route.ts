...
import { getLandingPageById, updateLandingPage, slugExists } from '@/lib/queries'
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!lp) return NextResponse.json({ error: '見つかりませんでした' }, { status: 404 })
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (await slugExists(body.slug, numId)) {
      return NextResponse.json({ error: 'このスラッグはすでに使われています' }, { status: 400 })
    }

    const lp = await updateLandingPage(numId, body)
    return NextResponse.json({ lp })
  } catch (e) {
    console.error(e)
    return NextResponse.json({ error: 'サーバーエラーが発生しました' }, { status: 500 })
